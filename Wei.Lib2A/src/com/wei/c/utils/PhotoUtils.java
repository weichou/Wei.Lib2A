...
public class PhotoUtils {
	...
	public static String savePictureToPhotoAlbum(Context context, Bitmap bitmap, String srcPath, String name, String description) {
		...
		try {
			...
			L.i(PhotoUtils.class, "MediaStore.Images.Media.insertImage: " + uri);
	...
	public static Session openSysCamara(Activity activity, int requestCode, String outputFilePath, CropArgs cropArgs) {
	...
	public static Uri onActivityResult(Activity activity, Session session, int requestCode, int resultCode, Intent data) {
		...
		L.i(PhotoUtils.class, "uri:" + uri);
		if (resultCode == Activity.RESULT_OK) {
			if (uri != null) {
				if (requestCode == session.requestCode) {
					L.i(PhotoUtils.class, "pick photo result: " + uri);
				...
				} else if (requestCode == session.cropArgs.requestCode) {
					L.i(PhotoUtils.class, "crop photo result: " + uri);
				}
			} else {
				if (requestCode == session.cropArgs.requestCode) {
					L.i(PhotoUtils.class, "crop photo result: " + uri);
					File outputFile = new File(session.cropArgs.outputFilePath);
					if (outputFile.exists()) {
						uri = Uri.fromFile(outputFile);
						L.i(PhotoUtils.class, "Uri.fromFile: " + uri);
					}
				}
			}
		}
		...
		public boolean boundsOutOfLimit(Context ctx, Uri uri) {
			...
			if (limitWidth > 0 && limitHeight > 0) {
				try {
					...
					L.w(PhotoUtils.class, "width:" + bounds[0] + ", height:" + bounds[1]);
